...
awk -i inplace 'BEGIN{found_video = 0} {if ($0 ~ " *## Video$") found_video = 1; else if ($0 !~ ".*#.*") found_video = 0; if (found_video) sub("# ", "", $0); print $0;}' "$confdir"/ranger/scope.sh && chmod +x "$confdir"/ranger/scope.sh
